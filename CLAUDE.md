...
- [2026-09-11] **기출 OCR 2차 — 검수량 줄이기·그림 제자리·쪽 넘김 복구**([sql/22](sql/22_problem_bank_merge_passages.sql), [sql/23](sql/23_problem_bank_figures.sql)). 되돌리지 말 것:
  ...
  - ⚠️ **오래 걸리는 일이 끝난 뒤의 본문을 읽는다**([useTrackedState](src/hooks/useTrackedState.ts)). '그림 추가' 를 누른 뒤 원본을 끌기까지, 파일이 올라가는 동안, 다음 쪽을 읽는 수십 초 동안 선생님은 계속 글을 친다. 시작 시점 값을 닫아 두면 그 사이 친 글이 **말없이 되돌아간다**. ref 는 **설정 함수 안에서만** 쓴다(`react-hooks/refs` 가 렌더 중 쓰기를 막는다)
