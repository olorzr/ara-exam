...
- [2026-09-11] **기출 OCR 2차 — 검수량 줄이기·그림 제자리·쪽 넘김 복구**([sql/22](sql/22_problem_bank_merge_passages.sql), [sql/23](sql/23_problem_bank_figures.sql)). 되돌리지 말 것:
  ...
  - ⚠️ **자리표시자 번호는 늘 그 판의 그림 목록 기준이다.** 좌표를 못 읽어 그림을 버릴 때는 **번호를 옮겨 붙이고**(`remapFigurePlaceholders`), 조각을 갈아 끼울 때는 **글과 그림을 함께** 간다. 한쪽만 바꾸면 1번 자리에 딴 그림이 그려진다 — **없는 것보다 나쁘다**(없는 것은 눈에 띄지만 딴 그림은 안 띈다). 버리는 판이 그림을 더 알아봤으면 조용히 넘기지 말고 경고한다. 문항도 같다 — 빈 발문을 갈아 끼울 때 그림을 함께 간다(`fillGaps`)
  ...
  - ⚠️ **중복이라 안 붙여도 그 조각의 키는 등록한다.** 안 하면 겹쳐 읽은 다음 묶음이 같은 이어짐을 또 냈을 때 표에서 못 찾고, 그 사이 `lastPage` 가 그 쪽까지 와 있어 `findOpenPassage` 도 거른다 — **주인 없는 지문이 하나 더 생기고** 처음 보는 문항이 거기 붙는다
