...
### 릴리스 공지 — 관리자시스템 공지사항 연동 [2026-09-14]
...
**스크립트가 알아서 하는 것**: 제목(`26년 9월 14일 시험관리 업데이트`)·카테고리(`업데이트`)·📌 고정.
같은 날 여러 번 배포하면 **한 공지에 병합**되고 `🔧 사소하지만 중요한 수정들` 섹션도 하나로 합쳐진다.
관리자시스템 자체 공지와는 제목 접미사로 갈려 서로 섞이지 않는다.
...
- 링크: 이 앱 화면은 절대주소 `https://test.araeducation.co.kr/...`(공지가 관리자시스템 안에서 렌더되므로),
  관리자시스템 화면은 `/grades/manual` 같은 상대주소
