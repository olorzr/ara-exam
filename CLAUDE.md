...
**공지는 하루에 하나다.** 관리자센터와 이 앱이 같은 날 배포돼도 글은 `26년 9월 15일 업데이트` 하나뿐이고,
어느 시스템 이야기인지는 본문 안의 구분선(`━━━ 📝 시험관리시스템 ━━━`)으로 갈린다.
...
```bash
node /Users/ara/Projects/Ara-system/scripts/post-update.js --app exam --dry-run "<HTML>"
node /Users/ara/Projects/Ara-system/scripts/post-update.js --app exam "<HTML>"
```
...
**스크립트가 알아서 하는 것**: 제목(`26년 9월 15일 업데이트`)·카테고리(`업데이트`)·📌 고정·구분선.
`--app exam` 은 **본문의 시험관리시스템 블록**을 고르는 것이지 제목을 바꾸지 않는다. 같은 날 여러 번
배포하면 **그 블록 안에서 병합**되고 `🔧 사소하지만 중요한 수정들` 섹션도 블록마다 하나로 합쳐진다 —
관리자센터 블록은 건드리지 않는다. 📌 고정은 가장 최근 공지 하나뿐이다.
...
- **구분선(`━━━ 📝 시험관리시스템 ━━━`)은 스크립트가 붙인다** — 본문에 직접 적지 말 것
