...
- [2026-09-14] **학교·학년·시험은 스캔마다 한 번만 묻는다 — 프린트 이름은 그 제목에 붙는다**([sql/29](sql/29_print_bundle_exam_meta.sql), [scan-meta.ts](src/lib/print-scan/scan-meta.ts)). 되돌리지 말아야 할 판단들:
  ...
  - ⚠️ **크게 보기는 문서를 한 번만 연다**([usePdfPagePreview.ts](src/hooks/usePdfPagePreview.ts)). 쪽을 넘길 때마다 열면 그때마다 파일 전체를 복사한다. 로딩은 state 가 아니라 **'무엇을 이미 그렸는가'(파일+쪽)에서 파생**한다 — 효과 안의 동기 setState 는 lint 가 막는다(아래 2026-09-08 ④). 여는 데 실패한 문서는 붙들지 않는다(다음 쪽도 같은 실패를 물려받는다)
