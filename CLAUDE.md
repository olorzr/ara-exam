...
- [2026-09-11] **기출 OCR 2차 — 검수량 줄이기·그림 제자리·쪽 넘김 복구**([sql/22](sql/22_problem_bank_merge_passages.sql), [sql/23](sql/23_problem_bank_figures.sql)). 되돌리지 말 것:
  ...
  - ⚠️ **중복이라 안 붙여도 그 조각의 키는 등록한다.** 안 하면 겹쳐 읽은 다음 묶음이 같은 이어짐을 또 냈을 때 표에서 못 찾고, 그 사이 `lastPage` 가 그 쪽까지 와 있어 `findOpenPassage` 도 거른다 — **주인 없는 지문이 하나 더 생기고** 처음 보는 문항이 거기 붙는다. ⚠️ 다만 그 키는 **가리키기만 하는 자리**(`FragmentRef.alias`)로 표시한다 — 가리키는 것이 그 조각이 아니라 통째로 담은 큰 조각이라, 나중에 더 긴 이어짐이 와서 갈아 끼우면 지문 앞부분과 그림이 통째로 날아간다
  ...
  - ⚠️ `exam.merge_passages` 는 두 행을 `FOR UPDATE` 로 **잠그고** 읽는다. 안 그러면 그 사이 남이 고친 본문을 덮어쓴다. 합친 뒤 `render_mode` 는 **글로 되돌린다** — 잘라 둔 이미지는 한쪽 쪽만 담고 있다. **그림 경로를 이어 붙이고 자리표시자 번호를 민다**([sql/24](sql/24_problem_bank_merge_figures.sql)) — 안 밀면 뒤 지문의 '1번' 이 앞 지문 그림을 가리킨다
