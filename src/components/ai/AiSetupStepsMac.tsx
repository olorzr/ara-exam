...
/**
 * 맥 설치 — 선생님이 하는 일은 **두 가지**뿐이다.
 *   1단계: Node.js 설치 (nodejs.org 설치 프로그램 더블클릭 — 터미널 없음)
 *   2단계: 터미널에 명령 한 줄 (codex 설치 + ChatGPT 로그인 + 자동 시작을 전부 한다)
 *
 * 왜 파일 다운로드가 아닌가: 브라우저로 받은 `.command` 는 실행 권한이 없어 더블클릭이
 * 실패하고, 서명 없는 스크립트는 Gatekeeper 가 우클릭-열기로도 안 풀어 준다.
 *
 * ⚠️ 붙여넣기를 **한 번으로 유지할 것.** 컴퓨터를 잘 모르는 선생님에게는 붙여넣기
 *   횟수가 곧 실패 지점이다.
 *
    ...
    <>
      ...
      <ol className="mt-4 space-y-3 list-decimal list-inside">
        <li>
          ...
          에서 <strong>macOS Installer</strong> 를 내려받아 <strong>더블클릭</strong>으로 설치합니다.
          여기는 터미널이 필요 없습니다.
        </li>

        <li>
          <strong>⌘ + 스페이스</strong> → <code className="rounded bg-gray-100 px-1 py-0.5">터미널</code> 을
          열고 아래 한 줄을 붙여넣습니다. <strong>나머지는 알아서 진행됩니다.</strong>
          <CopyCommand command={macInstallCommand(port)} />
          <p className="mt-2 text-xs text-gray-500">
            중간에 <strong>ChatGPT 로그인 창</strong>이 열립니다. 본인 계정으로 로그인해 주세요.
            학원 사이트가 아니라 OpenAI 공식 화면이며, 로그인 정보는 내 컴퓨터에만 저장됩니다.
          </p>
          <p className="mt-2 text-xs text-gray-500">
