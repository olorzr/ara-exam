...
import { CONCEPT_PICK_DEFAULT_COUNT, runConceptPick, type ConceptPick } from '@/lib/concept-pick';
...
export function useConceptPick({
  ...
  const [count, setCount] = useState(CONCEPT_PICK_DEFAULT_COUNT);
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (added.length === 0) {
        toast.warning('마킹할 용어를 찾지 못했어요. 개수를 줄이거나 본문을 확인해 주세요.');
      } else {
        // 못 붙인 것도 함께 알린다 — 안 그러면 "10개 추천" 이라고 하고 7개만 붙는다
  ...
  }, [editorRef, marks, count, addMarkByText]);
  ...
  return { count, setCount, running, applied, notFound, run, cancel, removeOne, undoAll };
}
