 ...
 * 꾸준히 나오는데 목차에 없는 것을 대분류로 더한 것이다(`남북한 언어`).
 *
...
const GRAMMAR_SOURCE: GrammarSource = {
  ...
  // 여기부터는 **교재 목차에 없고 중학 기출을 보고 더한** 대분류다. 목차 순서를 흐트러뜨리지
  // 않도록 뒤에 붙인다.
  //
  // 중3 '통일 시대의 우리말' 은 기출 62문항이 나오는 큰 단원인데 붙일 자리가 없었다.
  // `어휘 체계와 양상 > 표준어·방언` 에 넣어 보니 중1 지역 방언 문항과 한 칸에서 섞였고,
  // 표기·맞춤법 차이(두음법칙·사이시옷·띄어쓰기)는 애초에 '어휘' 가 아니다.
  '남북한 언어': [
