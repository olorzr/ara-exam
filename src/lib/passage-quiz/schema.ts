 ...
 *
 * 모양만 강제하고, 값이 **지문·참고자료와 맞는지**는 `parse.ts` 가 다시 본다.
 *
 * ⚠️ 근거의 **출처(`source`)는 스키마에 없다** — 모델에게 묻지 않고 파서가 대조로 정한다.
 *    물어서 받으면 그 말을 믿게 되는데, 지어낸 근거에 자료 이름만 붙여 오면 막을 길이 없다.
 *    (아래 두 인터페이스의 `source` 는 **파서가 채워 돌려주는** 필드다.)
 */
...
export interface OxItem {
  ...
  statement: string;
  answer: 'O' | 'X';
  /** 답의 근거가 되는 **지문·참고자료 그대로의** 구절 */
  evidence: string;
  /** 근거를 찾은 곳. `''` 는 지문, 그 밖은 참고자료 이름 — **파서가 대조로 정한다** */
  source: string;
}
