...
- 구역·상자는 <blockquote data-box="말머리">…</blockquote> 로 감싼다. 세 가지가 있다:
    ...
    (가) (나) (다) … (차) 글 구분 → data-box="가"
    [A] [B] 구간 표시            → data-box="A"
  값에는 **괄호를 넣지 않는다**(인쇄할 때 다시 붙인다).
  발문 아래 테두리 상자에 말머리가 안 찍혀 있어도(예문·대화·단어 목록) data-box="보기" 다 —
  말머리 없는 <blockquote> 는 상자로 인쇄되지 않는다.
  ㉠·ⓐ 같은 본문 기호와 밑줄은 구역이 아니다 — 글자로 그냥 둔다.
- 표는 <table><tbody><tr><td>….
- 글로 못 옮기는 **그림·표·사진**이 있으면 그 자리에 <figure data-figure="1"></figure> 를
  넣는다(비어 있는 태그다). 번호는 그 항목 안에서 1부터 나온 순서대로 붙인다.
  **<img> 는 쓰지 않는다** — 그림은 우리가 원본에서 잘라 그 자리에 끼운다.
...
[그림]
- 글로 옮길 수 없는 **그림·표·악보·도식·사진**은 figures 에 그 **부분만**의 위치를 적는다.
  항목 전체(box)가 아니다 — 그림만 잘라 본문의 제자리에 끼울 것이다.
- figures 의 순서가 곧 본문 자리표시자의 번호다: 첫 번째가 <figure data-figure="1">.
  **자리표시자와 figures 를 짝지어** 낸다. 하나만 내면 짝이 안 맞는다.
- 위치는 box 와 같은 방식이되 **page 가 하나 더 있다**(column·top·bottom·page).
  page 에는 **그 그림이 실제로 인쇄된 쪽 번호**를 적는다 — 쪽을 넘어가는 지문을 한 항목으로
  낼 때, 다음 쪽에 있는 그림을 시작 쪽 번호로 적으면 엉뚱한 자리가 잘려 들어간다.
- 그림의 **테두리를 넉넉히 감싸도록** 잡는다 — 좁게 잡아 축 이름이나 범례가 잘리면
  그 문항을 풀 수 없다.
- <table> 로 옮길 수 있는 **글자만 있는 표는 넣지 않는다.** 글로 적는 편이 낫다
  (검색도 되고 다시 조판할 수도 있다).
- 그림이 없으면 빈 배열([])로 둔다.
