...
--   따라서 상위 검색은 @>(contains) 가 아니라 **&&(overlaps)** 다 — 앱이 마스터에서 고른 마디와
--   그 아래 경로를 전부(중간 마디 포함) 펼쳐 넣는다
--   (src/lib/problem-bank/grammar-tree.ts 의 grammarPathsUnder).
--
-- ⚠️ 마스터는 DB 가 아니라 **코드 상수**다(grammar-tree.ts). 수능 문법 체계는 교재 목차대로
--   고정이고, 거기 없는 개념을 더할 때도 파일을 고쳐 배포하면 되므로(중학 기출의 '남북한 언어')
--   관리 화면·마스터 표를 두지 않았다. 저장하는 값은 그 이름 경로의 스냅샷이므로
