...
- 정의: 문항이 묻는 문법 개념을 **이름 경로 문자열의 목록**으로 스냅샷한 값
  ...
  체계는 수능 문법 교재 목차(6개 대분류 / 100개 핵심 개념) + 중학 기출을 보고 더한 대분류
  (`남북한 언어` — 중3 '통일 시대의 우리말', 어휘 차이·표기와 맞춤법 차이·동질성 3개념)이고
  마스터는 **코드 상수**다
- ⚠️ area_path·unit_path 와 **모양이 다르다**: 그쪽은 배열 하나가 경로 하나라 문항에 한 개만
  붙지만, 문법은 한 문항이 개념 두셋을 걸쳐서 경로를 `' > '` 로 이어 붙인 문자열을 원소로 담는다.
  그래서 상위 검색이 `contains`(@>)가 아니라 **`overlaps`(&&)** 다 — 고른 마디와 그 아래 경로를
  전부(중간 마디 포함) 펴서 찾는다. 중간 마디로 태깅된 종합 문항(`'단어 > 품사'`)도 걸려야 하기 때문이다
...
- ⚠️ 마스터로 만드는 까닭: 문법 태그는 업로드가 아니라 **나중에 손으로** 붙는 것이라,
  패싯(태깅된 경로)만 쓰면 '태그 0건 → 선택지 0건 → 붙일 길 없음' 이라는 닭-달걀이 된다
...
- ⚠️ **조상은 자손의 합이 아니다.** 한 문항이 같은 조상 아래 태그를 둘 달 수 있고(피동+사동)
  조상 자신으로 태깅된 문항도 있어서(`'단어 > 품사'`) 그냥 더하면 한 문항을 두 번 센다.
  문항마다 걸리는 경로를 집합으로 모아 **경로당 한 번만** 센다 —
  그래야 상위 검색(고른 마디와 그 아래 경로의 `overlaps`)의 결과 수와 맞는다
