...
- 문법 축만 **문항에 여러 개** 붙는다(`grammar_paths`, 원소 하나가 경로 하나). 그래서 저장 모양과
  조회 연산자가 다르다 — 상위 검색은 `grammarPathsUnder` 로 고른 마디와 그 아래 경로를
  (중간 마디까지) 펴서 `overlaps(&&)`,
...
- 훑는 축이 네 가지다: 교과서 단원 트리, **학교 기출 트리**(학교 › 학년도 › 학년 › 학기·시험),
  **작품 트리**(문학·비문학 › 지은이 › 작품, `work-tree.ts` — 작품명순으로 바꿀 수 있고
  그 선택은 localStorage 에 남는다), **문법 트리**(대분류 › 중분류 › 개념,
...
- 문법 건수는 **전역값**이다(다른 조건을 반영하지 않는다). 조상 건수는 자손 건수의 합이 아니라
  **문항 단위로 중복을 없앤** 값이다(`grammar-counts.ts`) — 한 문항이 같은 조상 아래 태그를
  둘 달 수 있고 조상 자신으로 태깅된 문항도 있어, 그냥 더하면 두 번 세어지고 그러면 `(n)` 이
  눌렀을 때의 목록 길이와 어긋난다
- 필터 줄의 칸 규칙('전체'·'미지정' 센티널, 조건이 걸린 칸을 남길지, 패싯에 없는 현재 값을
  끼워 넣을지)은 전부 `filter-axes.ts` 한 곳에 있다. 화면(`ProblemFilterBar`)은 그리기만 한다
- 작품 축의 값은 `problems.work_titles`(배열)이고 지문(`passages.works`)과의 동기화는 **DB 트리거**가
  한다(sql/33). 조회는 배열 포함(`contains`)이라 `(가)(나)` 를 함께 묻는 문항이 두 작품 어느 쪽으로
  훑어도 나오고, 작품을 골라 훑으면 지문 묶음 안에서 **'다른 작품과 함께 묻는 문항'** 이 갈린다
  (`splitByWorkSpan`). 옛 단수 컬럼(`work_title`·`passages.title`/`author`)은 **파생**으로 남아
  인쇄 스냅샷·검색 평문이 그대로 쓴다 — 앱은 보내지 않는다. 표기 정규화는 `work-title.ts` ↔
  `exam.normalize_works` 1:1
...
- 인쇄물 **머리글에는 출처를 모아 찍지 않는다**(문제지·교사용·답지 셋 다). `source_labels` 는
  계속 저장하지만 쓰는 곳은 문제지 **목록 화면**뿐이다
- 인쇄물은 **셋**이다(2026-09-21): 문제지 / 교사용(문항 밑에 정답·해설) / 답지(빠른 정답 격자
  다음에 해설). OMR 답안지는 쓰지 않아 뺐다. 문제지와 교사용은 `ProblemPaperView` 한 컴포넌트가
  `showAnswers` 로 가른다 — 두 벌로 두면 선생님 종이와 학생 종이의 문항 모양이 갈라진다
- 교사용의 **긴 해설(평문 400자 초과)은 문단 단위 블록으로 갈라** 흘려 보낸다. 문항 블록에
  담으면 한 쪽을 넘기는 순간 인쇄 엔진이 문항까지 통째로 축소한다(답 줄은 짧아서 그대로 둔다)
- 정답 표기(`③`·'미입력')와 해설 고르기는 `answers.ts` 한 곳이다 — 교사용·빠른 정답·답지
  해설 셋이 같은 함수를 쓴다
- 문항마다 **출처를 그 위에** 작은 글씨로 찍는다(`.pb-q__source`, `sourceLabel`). 학기까지
