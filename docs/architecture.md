...
- 인쇄물 **머리글에는 출처를 모아 찍지 않는다**(문제지·답지·OMR 답안지 셋 다). `source_labels` 는
  계속 저장하지만 쓰는 곳은 문제지 **목록 화면**뿐이다
- 인쇄물은 **넷**이다(2026-09-20): 문제지 / 교사용(문항 밑에 정답·해설) / 답지(빠른 정답 격자
  다음에 해설) / OMR 답안지. 문제지와 교사용은 `ProblemPaperView` 한 컴포넌트가
