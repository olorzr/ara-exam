...
## [0.1.15] - 2026-09-04
### Changed
- **넓은 표가 있어도 개념관리지 시트를 2단으로 되돌렸다.** 0.1.13 이 "열 3개 이상 표가 있으면 시트 전체를 1단" 으로 강제한 건 열 폭을 못 맞춰 넓은 표가 2단 칸(≈328px)에서 깨지던 시절의 임시 조치였다. 0.1.14 의 열 폭 맞춤이 칸 폭에 맞춰 주므로 근거가 사라졌고, 1단은 페이지 수만 늘린다. 단 수는 다시 **글자 수(300자 초과 → 2단)만** 본다([src/lib/print/sheet-columns.ts](src/lib/print/sheet-columns.ts) 의 `decideSheetColumns`, `maxTableColumns` 삭제)
