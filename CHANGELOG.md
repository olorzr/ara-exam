...
### Changed
- **대시보드 네 번째 카드가 '개념지' 대신 '총 문제 수'를 보여 줘요.** 문제 은행에 쌓인
  기출 문항이 모두 몇 개인지 대시보드에서 바로 보입니다.

## [0.9.14] - 2026-09-22

### Fixed
- **「모진 소리」·「따뜻한 말」 지문에 빠져 있던 (다) 「거울」을 붙였어요.** 시 두 편과 함께
  실린 조은서 학생의 소설인데 작품 목록에 없었습니다. 발문이 (다)를 짚는 문항 7개에만
  달았어요 — (가)(나)만 묻는 문항은 그대로입니다.

### Added
- **학교별 교과서와 교과서 수록작 목록을 문서로 남겼어요.** 앞으로 작품명이 비어 있을 때
  지어내지 않고 이 목록과 대조합니다(행당·광희·옥정중 천재 박영목 / 무학·동마·마장중
  천재 노미숙 / 한대부중 미래엔).

## [0.9.13] - 2026-09-22

### Fixed
- **작품명이 비어 있던 지문 37개를 채웠어요.** 광희중 시험지는 지문 끝에 출처를 잘 안 찍어서,
  같은 글인데도 행당중 것만 이름이 붙어 있었어요. 본문을 맞대어 이름을 되찾았습니다 —
  「통일 시대의 우리말」, 「시계는 어떻게 달력을 이겼을까?」, 「잊힐 권리의 법제화」,
  「신조어를 무분별하게 사용하지 말자」, 「위대한 명연설」 등이에요.
- **「간송 전형필」(이충렬)이 이제 작품 목록에 떠요.** 지문 3개가 다 이름 없이 있었습니다.
- **두 학교 교과서가 같은데 작품명이 다르게 쌓여 있던 것 셋을 바로잡았어요.** 광희중 쪽이
  틀린 이름이었습니다 — 「우리 동네 예술가들」→「길모퉁이에서 만난 사람」(양귀자),
  「아들아」→「어머니는 왜 숲속의 이슬을 털었을까」(이순원),
  「내가 읽은 책과 세상」→「맛있는 책, 일생의 보약」(성석제).
- **「지구가 뿔났다」로 적혀 있던 지문을 「거울 뉴런」으로 고쳤어요.** 「지구가 뿔났다」는
  남종영 작가의 책 이름이고, 시험지가 그 출전을 작품명 자리에 찍어 둔 것이었습니다.
- 「시계와 달력」도 「시계는 어떻게 달력을 이겼을까?」(안광복)로 합쳤어요.

## [0.9.12] - 2026-09-22

### Fixed
- **같은 작품이 이름만 달라 여러 번 뜨던 것을 합쳤어요.** 「동해 바다 - 후포에서」가 붙임표·
  띄어쓰기 차이로 셋, 「한중일 삼국의 젓가락」과 「정전기가 겨울로 간 까닭은」이 각각 둘로
  갈려 있었습니다. 시험지에 인쇄된 출처 표기(`- 김경은, 「한중일 삼국의 젓가락」`)를 따라
  하나로 모았고, 딸린 문항 47개가 함께 따라왔어요.
- **〈보기〉에만 나오는 글은 이제 작품 목록에 서지 않아요.** 「그린 북」은 「하늘은 맑건만」을
  푸는 문항의 참고 자료일 뿐이라 그 작품 안에만 남겼습니다. 「우리말 사랑 1」도 같은 경우예요.
  앞으로 올리는 시험지에도 같은 규칙이 적용됩니다.

### Changed
- **작품 목록에 '문법' 칸이 생겼어요.** 「훈민정음」처럼 문법을 묻는 글이 비문학 폴더에
  섞여 있었는데, 이제 문학 / 비문학 / 문법 / 영역 미지정 네 칸으로 나뉩니다. 독서 지문을
  훑을 때 문법 자료가 걸리적거리지 않아요.

## [0.9.11] - 2026-09-22

### Changed
- **(가)(나)로 나뉜 지문의 작품을 각각 적어 두었어요.** 시 두 편이 함께 실린 지문이 그동안
  한 편만 기록돼 있었는데, 이제 `(가) 엄마 걱정 · 기형도` `(나) 딸기 · 이재무` 처럼 구역마다
  따로 보입니다. 광희중·행당중 문학 지문 213개를 전부 확인해 118개를 고쳤어요.
  - **문항도 자기가 묻는 작품만 달고 있어요.** `(가)의 화자에 대한 설명으로…` 처럼 한 구역만
    묻는 문항은 그 작품 하나만 붙습니다. 작품으로 훑을 때 상관없는 문항이 딸려 나오지 않아요.
  - 「소나기」가 소설과 드라마 대본으로 갈려 작품 목록에 여러 번 뜨던 것도 정리했습니다.
  - 지은이가 한 칸씩 밀려 있던 지문 하나도 바로잡았어요(「엄마야 누나야」가 한용운으로 적혀
    있었습니다).
  - 앞으로 올리는 시험지는 AI가 알아서 구역별로 적어 줍니다.

### Removed
- **시험 삼아 올렸던 기출 3개를 지웠어요.** 무학중·동마중·성수고 시험지입니다. 문제 은행에는
  이제 광희중·행당중 기출만 남아요.
  - 그 문항이 들어가 있던 문제지 두 개(`행당중 1학년 품사`, `광희중 2 2학기 중간 직보`)에서도
    해당 문항을 빼고 번호를 다시 매겼습니다.


## [0.9.10] - 2026-09-21

### Added
- **문제 아카이브 '작품' 목록이 문학과 비문학으로 나뉘어요.** 설명문·강연 자료처럼 제목만 적혀
  있던 글이 작품 사이에 섞여 있었는데, 이제 `문학`·`비문학` 폴더로 갈라집니다. 갈래는 그 글이
  실린 **지문의 영역**으로 가려요 — 지은이가 없는 『홍길동전』도 문학으로, 지은이가 있는
  『통일 시대의 우리말』도 비문학으로 제대로 들어갑니다.
  - 영역을 아직 안 붙인 지문의 작품은 `영역 미지정` 폴더에 모여요. 검수 화면에서 영역을 붙이면
    문학·비문학으로 옮겨 갑니다.
- **작품을 지은이순으로 볼지 작품명순으로 볼지 고를 수 있어요.** '작품' 탭 위의 정렬 칸에서
  바꾸면 되고, 작품명순으로 보면 지은이 폴더 없이 `동백꽃 — 김유정 (14)` 처럼 한 줄로 쭉
  나열됩니다. 고른 정렬은 그 컴퓨터에 기억돼 다음에 열어도 그대로예요.

## [0.9.9] - 2026-09-21

### Added
- **단어 시험을 객관식으로 보면 OMR로 자동 채점돼요.** 시험을 만들면 학원에서 쓰는 **90A 답안지
  양식**이 자동으로 연결됩니다. 학생이 90A에 마킹하면 관리자센터 > OMR 채점에서 스캔만 뜨면 돼요.
  - **주관식(뜻 보고 단어 쓰기)도 그대로 쓸 수 있어요.** 채점 형식은 시험을 만들 때가 아니라
    **채점할 때** 고릅니다 — OMR 채점 화면에 들어가면 객관식으로, 수기 성적 기입에서 되돌리면
    주관식으로 정해집니다.
- **문항이 90개를 넘으면 시험지가 답안지 장을 안내해요.** 91번·181번 앞에 `여기부터 답안지 2장째`
  구분 줄이 들어가고, 문항 번호 옆에 `2-1`(2장째 1번 칸)이 작게 붙습니다. 머리글에도
  `답안지 2장`이 찍혀요. 스캔은 학생별로 1장·2장 순서대로 넣으면 관리자센터가 합쳐서 한 성적으로
  저장합니다.

### Fixed
- **단어가 1,000개를 넘는 카테고리 조합에서 시험이 조용히 잘리던 문제**를 고쳤어요. 이제 상한을
  넘으면 만들기 전에 알려주고 막습니다. (전에는 앞 1,000개만 담긴 시험지가 아무 표시 없이
  만들어졌어요)

## [0.9.8] - 2026-09-21

### Added
- **같은 지문의 문항이 한 상자로 묶여 보여요.** 문제 아카이브와 문제지 조합 목록에서 지문에
  딸린 문항들이 지문 머리(작품·출처·문항 수) 아래로 모입니다. 그동안은 어느 문항이 같은 글에
  딸린 것인지 목록만 봐서는 알 수 없었어요.
  - **문제지 조합에서는 묶음 머리의 '이 지문 담기' 로 한 번에 담을 수 있어요.** 화면에 보이는
    문항만이 아니라 **그 지문의 문항을 전부** 담습니다 — 조건에 안 걸렸거나 다음 쪽에 있는
    문항도 함께요.
- **문항을 눌러 뜬 창에서 바로 문제지에 담을 수 있어요.** 아래에 '문제지에 담기' 단추가
  생겼습니다. 지문이 있는 문항은 **'이 지문의 문항 N개 담기'** 도 함께 나와서, 딸린 문항을
  빠뜨리지 않고 담을 수 있어요. 담으면 창이 닫히고 오른쪽에 바로 보입니다.
  - 문제 아카이브에서는 담을 곳이 없어 이 단추가 나오지 않아요.

### Changed
- **목록 정렬이 시험지에 실린 차례를 따라가요.** 최근에 올린 시험지가 위에 오는 건 그대로인데,
  **같은 시험지 안에서는 1번부터 차례대로** 나옵니다. 그동안 한 시험지의 문항이 뒤죽박죽 섞여
  나와서 같은 지문의 문항이 목록 여기저기 흩어져 있었어요.
  - 문항이 50개를 넘는 시험지가 **뒤쪽 문항부터** 나오던 것도 함께 고쳤어요(새로 올리는
    시험지부터 적용돼요).
- 지문을 불러오는 동안 '지문을 불러오지 못했어요' 가 잠깐 뜨던 것을 고쳤어요.

## [0.9.7] - 2026-09-21

### Changed
- **답지의 복수 정답을 `③, ⑤` 로 찍어요.** 그동안 정답이 둘인 문항만 `1,4` 라는 날숫자로
  나와서, 한 격자 안에 `③` 과 `1,4` 가 섞여 있었습니다. 이제 다른 칸과 같은 모양입니다.
  - **교사용 시험지와 아카이브 화면에서도 정답 선지 둘에 모두 표시가 붙어요.** 복수 정답
    문항은 지금까지 **어느 선지에도 색이 안 칠해졌습니다** — 답지와 교사용이 어긋나
    있었어요.
  - 정답을 `1,0` 이나 `1,6`(다섯 개뿐인데 여섯째) 처럼 잘못 적어 둔 문항은 **적힌
    그대로** 찍습니다. 억지로 반만 읽어 멀쩡한 정답처럼 인쇄하지 않아요 — 보고
    고치실 수 있게 남깁니다.

### Removed
- **'OMR 답안지' 탭을 없앴어요.** 학생이 답을 옮겨 적는 빈 종이인데 쓰지 않아서 뺐습니다.
  문제지 화면은 이제 **문제지 / 교사용 / 답지** 셋입니다. 답지(빠른 정답 + 해설)는
  그대로예요.

## [0.9.6] - 2026-09-21

### Added
- **잘못 잘린 그림을 원본에서 다시 잡을 수 있어요.** 검수 화면의 그림 미리보기마다 오른쪽
  아래에 ✂ 단추가 생겼습니다. 누르면 왼쪽 원본이 그 문항이 실린 쪽으로 넘어가고, 거기서
  끌어 잡으면 **그 번호 자리의 그림만** 새것으로 바뀌어요.
  - 본문의 `🖼 그림 n` 칩은 **그대로 있습니다** — 자리를 다시 옮기지 않아도 돼요. 그동안은
    ✕ 로 빼고 다시 붙여야 해서 그림이 맨 뒤로 가고 번호가 밀렸습니다.
  - 잡는 동안 그 문항·지문의 영역이 원본 위에 **점선**으로 표시돼요. 어디를 잡아야 하는지
    찾기 쉽습니다.
  - **'만들지 못했어요' 로 비어 있던 자리도** 같은 방법으로 채울 수 있어요.
  - 이미 만들어 둔 문제지는 그대로입니다 — 옛 그림 파일은 남겨 두고 새 파일을 올려요.
  - ⚠️ **'이미지로 출제' 중인 문항·지문**은 인쇄에 잘라 둔 원본 이미지가 나가서, 그림을
    고쳐도 새 문제지에 반영되지 않아요. 그런 항목에는 안내가 함께 뜹니다 — '글로 출제' 로
    바꾼 뒤에 고쳐 주세요.

## [0.9.5] - 2026-09-20

### Fixed
- **〈보기〉 상자가 사라져 보이던 기출 문항을 고쳤어요.** 시험지에서 상자로 묶여 있던 예문·대화·
  단어 목록이 아카이브와 문제지에서 테두리도 말머리도 없이 **들여쓰기만 된 글**로 찍혔습니다
  (2025 동마중 중1 2학기 중간 22·25·28~30번을 비롯해 7개 시험 15문항). 전부 〈보기〉 상자로
  바로잡았고, 이미 만들어 둔 문제지('행당중 1학년 품사')에도 그대로 반영했어요.
  - 동마중 22·23번이 함께 보는 (가)~(마) 상자는 [22~23] 지문으로 옮겨 두 문항 앞에 한 번만 나옵니다.
  - 2025 무학중 중1 2학기 중간 9·12·15·22·23·27번은 발문이 '〈보기〉에서 … 고른 것은?' 인데
    상자 자체가 빠져 있었습니다. 원본 시험지를 보고 채워 넣었고, 27번 선지의 '돌레길' 오독도
    '둘레길' 로 고쳤어요.
  - 동마중 지문 머리글 둘이 '14~16]다음글을읽고물음에답하시오.' 처럼 안내문까지 붙어 있던 것을
    범위만 남겼습니다.
- **앞으로 읽어 오는 기출은 말머리 없는 상자도 〈보기〉로 들어와요.** 시험지에 '〈보기〉' 글자가
  없는 상자를 AI 가 맨 상자로 내던 자리입니다. 혹시 남더라도 이제 인쇄물에 왼쪽 선이 그어져
  '묶인 글' 로는 보입니다.
- **(바)·(사)… 글 구분이 살아나요.** 지문을 (가)~(마)까지만 가를 수 있어서 (바)부터는 머리글이
  사라졌습니다. 이제 (차)까지 됩니다 — 2023 행당중 중1 2학기 기말 '하늘은 맑건만', 2023 행당중
  중2 1학기 기말 편지글, 2021 행당중 중2 1학기 기말 '양반전' 지문에 바로 반영했어요.

## [0.9.4] - 2026-09-20

### Changed
- **문항과 문항 사이가 훨씬 넉넉해졌어요.** 앞서 한 번 띄웠는데도 여전히 다닥다닥 붙어
  보여서, 이번에는 **4~5배**로 크게 벌렸습니다. 문제를 풀다 눈이 옆 문항으로 미끄러지지
  않고, 학생이 필기할 자리도 생겨요.
  - 그만큼 **쪽 수는 늘어납니다** — 여백이 커진 만큼 한 장에 들어가는 문항이 줄어요.
  - 교사용도 같습니다. 해설이 긴 문항은 해설 끝에서 간격이 지므로 문제와 그 해설은
    붙어 있어요.
  - O,X·단답형 시험지와 학교 프린트 문답 시험지, 문제 보기 창은 예전 그대로입니다.

## [0.9.3] - 2026-09-20

### Added
- **문제지에 폴더를 통째로 담을 수 있어요.** 왼쪽 트리에서 학교·작품·단원을 고른 뒤
  '이 폴더 전체 담기' 를 누르면 조건에 걸린 문항이 한 번에 들어갑니다. 쪽을 넘겨 가며
  ＋ 를 누르지 않아도 돼요.
  - 화면에 60개만 보여도 **조건에 걸린 전부**가 담기고, 같은 지문의 문항은 붙어서 들어갑니다.
  - 한 장에 담을 수 있는 200문항을 넘는 폴더는 담기 전에 알려 드려요(잘라 담으면 지문에
    딸린 문항이 중간에서 끊깁니다).
- **체크박스로 골라 담기.** '골라 담기' 를 누르면 문항마다 체크박스가 생기고,
  '이 쪽 전체 선택' 으로 한 쪽을 통째로 고를 수도 있어요.
- **교사용 문제지가 생겼어요.** 문항 바로 밑에 정답과 해설이 붙어 있어서 채점하며 따로
  답지를 펼 필요가 없습니다. 객관식은 정답 선지에 표시가 붙고, 학생이 쓸 답 줄과
  이름·점수란은 빠집니다.
- **교사용의 긴 해설도 또렷하게 나와요.** 해설이 길면 한 쪽을 넘기면서 문항까지 통째로
  작아지던 자리인데, 이제 해설이 다음 쪽으로 자연스럽게 이어집니다(첫 줄에 문항 번호를
  달아 두어 쪽이 갈려도 어느 문제 해설인지 알 수 있어요).
- **답지에 해설이 실려요.** 맨 위에 **빠른 정답**이 먼저 나오고, 그 아래에 해설이 붙습니다.
  해설이 달린 문항이 하나도 없으면 빠른 정답만 나와요.
  - ⚠️ 지금은 올라온 기출에 해설이 저장된 문항이 **하나도 없습니다.** 교사용과 답지의 해설
    자리는 해설이 쌓이기 전까지 비어 있고, 정답은 그대로 나옵니다.

### Changed
- **출처가 문항 바로 위에 작은 글씨로 나와요.** 예전에는 문항 **아래**, 선지 다음이라
  어느 문항의 출처인지 헷갈렸습니다.
  - 출처에 **학기**가 함께 찍혀요 — `2026 중2 상현중 1학기 중간`. '중간' 만으로는 1학기인지
    2학기인지 알 수 없었습니다. (이미 만들어 둔 문제지는 그대로 학기 없이 인쇄됩니다.)
- **발문(문제 부분)이 굵게 나와요.** 〈보기〉 상자·표·선지는 그대로라, 무엇을 묻는지가
  한눈에 들어옵니다.
- **문항 사이가 넉넉해졌어요.** 너무 다닥다닥 붙어 있어 눈이 미끄러지던 자리입니다.
- **정답표를 '답지' 로, 답안지를 'OMR 답안지' 로 이름을 바꿨어요.** 답지에 해설이 들어가면서
  둘의 쓰임이 갈렸습니다.
- **정답이 ③ 처럼 선지 기호로 찍혀요.** 문제지에는 `①②③` 인데 정답표만 `3` 이라
  눈으로 맞출 때마다 세어야 했습니다.

## [0.9.2] - 2026-09-19

### Changed
- **문제지를 만들면 출처가 기본으로 찍혀요.** 그동안은 '출처 표시' 를 켜야 문항 아래
  `2025 상현중 중간` 이 나왔는데, 이제 켜 두는 것이 기본이고 체크박스가 **'출처 숨기기'**
  로 바뀌었습니다. 출처를 빼고 뽑고 싶을 때만 체크하시면 돼요.
  - 이미 만들어 둔 문제지는 그대로입니다 — 출처 없이 만든 문제지가 갑자기 출처를 달지 않아요.
- **문제지 맨 위의 출처 줄을 뺐어요.** 머리글에 담긴 출처를 죽 늘어놓던 줄인데, 출처는
  문항마다 그 자리에 찍히니 위에서 또 밝힐 자리가 아니었습니다. 정답표·답안지도 같습니다.
  - 그 자리에 적을 것이 하나도 없으면 **테두리 줄도 함께 사라집니다** — 정답표 맨 위에
    빈 띠만 인쇄되지 않아요.
